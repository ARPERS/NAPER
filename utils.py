...
def float_to_bins(num): 
    return format(struct.unpack('!I', struct.pack('!f', num))[0], '032b')

def bins_to_float(binary): 
    return struct.unpack('!f',struct.pack('!I', int(binary, 2)))[0]

def float_to_bint(num): 
    return struct.unpack('!I', struct.pack('!f', num))[0]

def bint_to_float(binary):
...
def flip_prob(model, p=0.1, debug=False, seedint=113, faulty_params=None):
    random.seed(seedint)
    res_model = copy.deepcopy(model)
    total_bit = sum(p.numel() for p in res_model.parameters())*32
    counter = 0
    if faulty_params is None:
        faulty_params = []
    with torch.no_grad():
        param_id = 0
        for param in tqdm(res_model.parameters()):
            if param_id in faulty_params:
                param_id += 1
                continue
            param_faulty = False
            temp = param.detach().reshape(-1).cpu()
            ori = param.shape
            ...
            temp = temp.reshape(ori).to("cuda")
            param.copy_(temp)
            if param_faulty and param_id not in faulty_params:
                faulty_params.append(param_id)
            param_id += 1
    if debug:
        print("Total bit:", total_bit," Flipped:", counter)
    return res_model, faulty_params

def flip_n(model, n=1, debug=False, seedint=113):
    random.seed(seedint)
    res_model = copy.deepcopy(model)
    total_bit = sum(p.numel() for p in res_model.parameters())*32
    counter = 0
    fault = set(np.random.choice(np.arange(1, total_bit), size=n, replace=False).flatten())
    with torch.no_grad():
        for param in tqdm(res_model.parameters()):
            temp = param.detach().reshape(-1).cpu()
            ori = param.shape
...
def ens_evaluate_acc(wrappermodel=None, models=None, testloaders=None, device=device, single=False,
                     time_profiler=False, full_profiler=False, debug=False, model_type=None, limit_model=2, limit_time=1e9):
    with torch.no_grad():
        test_sample = 0    
        test_correct = 0
        times_ret = []
        if single:
            ...
            # wrappermodel==none for ensemble without wrapper (for ensemble accuracy check),
            #                           format time [model1, model2, model3, ...]
            # if using wrappermodel format time [model1, model2, model3, ..., ALL]
            sample_input= next(iter(testloaders))[0][0].unsqueeze(0).to(device)
            label = next(iter(testloaders))[1][0].unsqueeze(0).to(device)
            ...
            if wrappermodel==None: 
                ...
                # with wrapper: Ensemble with wrapper only NAPER     
...
def eval_model_load(model_type, ids, model_name, models_path, device=device, compile=True, cifar100=False, traffic=False):
    m = len(ids)
    models = []
    for i in range(m):
        if cifar100:
            model_name_path = "resnet100_"+model_type+"-"+str(ids[i])
        else:
        ...
        if cifar100:
            model.linear = torch.nn.Linear(in_features=64, out_features=100, bias=True) # number of class labels
        if traffic:
            model.linear = torch.nn.Linear(in_features=64, out_features=43, bias=True) # number of class labels
...
def getstats(model, testloaders, ensemble=False, runs=10):
    ...
    for _ in range(runs):
        ...
        model._clear_errors()
        if ensemble:
            ret = ens_evaluate_acc(wrappermodel=model, testloaders=testloaders, single=True, time_profiler=True)
            inf_time.append(ret[-1])
        else:
            ret = evaluate_acc(model, testloaders, single=True, time_profiler=True) 
            inf_time.append(ret[-1])
    ...
    for k in range(runs):
        ...
        if ensemble:
            ...
            ret = ens_evaluate_acc(wrappermodel=model, testloaders=testloaders, single=True, time_profiler=True, limit_model=1)
            inff_time.append(ret[-1])
        else:
    ...
    model.protected = False
    return np.mean(inff_time), np.mean(inf_time) # msecond
